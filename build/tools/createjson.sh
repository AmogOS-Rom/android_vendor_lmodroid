...
existingOTAjson=./vendor/ota_config/$1.json
output=$2/$1.json
...
if [ -f $existingOTAjson ]; then
	...
	download="https://sourceforge.net/projects/amogos-rom/files/'$device'/'$v_max'.x/'$4'/download"
	...
	"response": [
		{
			...
			"device": "'$device'",
			"filename": "'$filename'",
			"download": "https://sourceforge.net/projects/amogos-rom/files/'$1'/'$v_max'.x/'$3'/download",
			"timestamp": '$timestamp',
			"md5": "'$md5'",
			"sha256": "'$sha256'",
			"size": '$size',
	...
	echo 'Consider adding official support by reading the documentation at https://github.com/amogos-rom/ota_config/blob/thirteen/README.md' >> $output;
fi
